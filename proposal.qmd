...
</br>The dataset contains 11 columns. The numerical columns: FIPS_Code, Year, Month, Percent_of_State_Areas_Population, Percent_of_Labor_Force_Employed_in_State_Area, and Percent_of_Labor_Force_Unemployed_in_State_Area are represented by the numeric data type in R. The categorical columns: State_Area, Total_Civilian_Non_Institutional_Population_in_State_Area, Total_Civilian_Labor_Force_in_State_Area, Total_Employment_in_State_Area, and Total_Unemployment_in_State_Area are represented by the character data type in R. The dataset has 11 variables and 29,892 entries, indicating a dimension of 29,892 x 11.
...
How has the unemployment rate in each U.S. state fluctuated over time, and what are the trends during significant economic events (e.g., the 2008 financial crisis, the COVID-19 pandemic)? Which states display higher resilience or vulnerability in unemployment trends?
...
1.  **Data Preparation:** We plan on taking the unemployment data by year and month to create a time series for each state, ensuring a consistent timeline. This will allow us to detect patterns and seasonality in unemployment rates.

2.  **Exploratory Data Analysis:** Calculate summary statistics such as mean, median, and standard deviation of unemployment rates for each state and year to understand the typical values and range of variation. Outlier detection will also be done on this data set.

3.  **Data Visualization:** Creating an animated choropleth map that displays unemployment rates by state for each month or year. The color scale will represent the unemployment rate, with darker colors indicating higher unemployment. This will provide a powerful representation of regional patterns in unemployment. Viewers will be able to see which states or regions are hit harder during recessions, offering insights into regional economic resilience.
...
| 3 | Question 1 - Problem statement and code | Pending | Rony, Nithin | Oct-02 | High | Prepare detailed explanation of what Question 1 is going to be, what columns are required and what we can conclude from the question. Based on what is to be answered, start writing code. Get it reviewed from other team members and push the changes |
...
| 5 | Question 1 - Review | Pending | Disha, Sharan | Oct-09 | Moderate | Review the code written by other 2 members and suggest changes |
| 6 | Question 2 - Review | Pending | Rony, Nithin | Oct-09 | Moderate | Review the code written by other 2 members and suggest changes |
